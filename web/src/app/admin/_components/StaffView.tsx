import { Users, UserPlus, Power, AlertTriangle, Trash2, CalendarClock, ChevronDown, KeyRound } from 'lucide-react'
...
export function StaffView({ staffList, currentUserId, targetMonthStr }: {
  staffList: any[]; currentUserId: string; targetMonthStr: string
}) {
  return (
    <>
      {/* スタッフ管理 */}
      <section className="animate-in fade-in slide-in-from-bottom-4 duration-500 max-w-3xl">
        <h2 className="text-xl font-semibold flex items-center gap-2 border-b border-white/10 pb-3 mb-6">
          <Users className="w-5 h-5 text-cyan-400" /> スタッフ管理
        </h2>
              ...
              <div className="p-4 pt-2 border-t border-white/10 bg-black/20">
                <form action={updateStaffAccount} className="space-y-4">
                  ...
                  <div className="flex justify-between items-center mt-4">
                    <button type="submit" className="bg-cyan-600 hover:bg-cyan-500 text-white font-bold py-1.5 px-6 rounded-lg transition-colors text-sm shadow-md">変更を保存</button>
                  </div>
                </form>

                <div className="flex items-center gap-3 mt-4 pt-4 border-t border-white/5">
                  {staff.id !== currentUserId && staff.role !== 'admin' && (
                    <form action={toggleStaffActive.bind(null, staff.id, staff.is_active)}>
                      <button type="submit" className={`flex items-center gap-1.5 px-3 py-1.5 rounded-lg transition-all shadow-md text-xs font-bold ${staff.is_active ? 'bg-red-500/20 text-red-400 border border-red-500/30 hover:bg-red-500/30' : 'bg-green-500/20 text-green-400 border border-green-500/30 hover:bg-green-500/30'}`}>
                        <Power className="w-3.5 h-3.5" /> {staff.is_active ? 'アカウントを凍結' : '凍結を解除'}
                      </button>
                    </form>
                  )}
                  {staff.id !== currentUserId && (
                    <form action={deleteStaffAccount}>
                      ...
                      >
                        <Trash2 className="w-3.5 h-3.5" /> アカウントの削除
                      </ConfirmButton>
                    </form>
                  )}
                </div>
        ...
        <div className="mt-8 pt-8">
          <div className="bg-white/5 border border-white/10 rounded-2xl p-6">
            <h3 className="text-lg font-bold mb-4 flex items-center gap-2">
              <UserPlus className="w-5 h-5 text-emerald-400" /> 新規スタッフの登録
            </h3>
            <form action={createNewStaffAccount} className="space-y-4">
              ...
              <button type="submit" className="bg-emerald-600 hover:bg-emerald-500 text-white font-bold py-2 px-6 rounded-xl transition-colors text-sm shadow-md">アカウントを発行する</button>
            </form>
          </div>
        </div>
      </section>
      ...
      <section className="animate-in fade-in slide-in-from-bottom-4 duration-500 max-w-3xl mt-12 border-t border-white/10 pt-10">
        <div className="space-y-4">
          <h2 className="text-xl font-semibold flex items-center gap-2 border-b border-white/10 pb-3 text-emerald-400">
            <CalendarClock className="w-5 h-5 text-emerald-400" /> 月次実績記録表・スケジュール表の出力
          </h2>
          <div className="p-6 rounded-2xl bg-emerald-950/20 border border-emerald-500/20">
            <p className="text-emerald-400/80 text-sm mb-4">浦安市提出の所定フォーマットを自動生成し、Excelファイルとしてダウンロードします。</p>
            <form method="GET" action="/api/export" className="flex flex-col sm:flex-row gap-3">
              <input type="month" name="month" defaultValue={targetMonthStr} className="bg-slate-900 border border-slate-700 text-sm px-4 py-2 rounded-xl focus:ring-2 focus:ring-emerald-500 outline-none" />
              <button type="submit" className="bg-emerald-600 hover:bg-emerald-500 text-white font-bold px-6 py-2 rounded-xl transition-colors text-sm shadow-md">出力 (.xlsx)</button>
            </form>
          </div>
        </div>
      </section>
